...
## Шаг 4 — базовое наполнение и первый вход в админку

После первого запуска база пустая: миграции создают таблицы, но не данные.
Один раз выполни базовый сид — он создаёт админа из `SEED_ADMIN_EMAIL` /
`SEED_ADMIN_PASSWORD`, шапку/подвал и статические страницы (Impressum,
Datenschutz, Verband, Kontakt …). Выдуманных игроков и матчей он **не** создаёт:

```powershell
docker compose -f infra\compose.prod.yml --env-file .env.production exec app pnpm seed:base
```

Потом открой `https://bandy-bund.de/admin`, войди этим логином и сразу смени
пароль в своём профиле. Команду можно повторять — она ничего не дублирует.

## Шаг 5 — реальные игроки и матчи

Фото игроков и файл с матчами кладутся в папку `import\` в корне репозитория
(формат и примеры — в `import\README.md`), потом:
...
Всё создаётся **черновиками** — на сайте появится только после публикации в
`/admin`. Отчёт в конце перечисляет, что проверить.

Позже поменять только фотографии (игроки уже поправлены и опубликованы):
положи новые файлы с теми же именами в `import\players\` и запусти
`pnpm import:portraits` той же командой — подробности в `import\README.md`.

Для локальной разработки есть ещё `pnpm seed` — базовый сид плюс выдуманные
демо-данные (Ян Ковальски и т. п.). **Не запускай его на сайте, который видят
реальные посетители.**
